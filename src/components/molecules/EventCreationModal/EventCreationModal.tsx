...
export const EventCreationModal: React.FC<EventCreationModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onEventCreate({
      ...
      assignedTasks: [],
      relatedElements: [],
      color: getDefaultCategoryColor(formData.category),
    })
    onClose()
    setFormData({
      title: '',
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex gap-3 mt-6">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-2 px-4 border border-border text-foreground rounded-lg hover:bg-muted transition-colors"
            >
              Cancel
            </button>
            <button
